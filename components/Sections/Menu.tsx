...
function Menu() {
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center text-[#f3e1c6] px-4 pt-12 pb-16 sm:pt-20 sm:pb-24 relative z-10">
      
      {/* Heading */}
      <div className="w-full md:w-2/3 text-center">
        <h1 className="text-2xl sm:text-3xl md:text-[4rem] leading-snug sm:leading-tight font-serif font-extrabold">
          A MASTERPIECE IS <span className="block text-[#dab67d]">GASTRONOMIC JOURNEY</span> THROUGH FLAVORS
        </h1>

        <p className="mt-4 text-xs sm:text-sm md:text-base max-w-[90%] md:max-w-[60%] mx-auto leading-relaxed tracking-wide text-[#f1d6b8]">
          EXPERIENCE OUR CHEF'S SPECIALTY DISHES — CRAFTED WITH PASSION, FLAVORS, AND FINEST INGREDIENTS THAT TELL A STORY FROM RAJASTHAN'S ROYAL KITCHENS.
        </p>
      </div>

      {/* Menu Cards */}
      <div className="w-full md:w-[70%] mt-10 flex flex-col md:flex-row justify-between gap-6 sm:gap-10 px-2 sm:px-4">
        {[leftMenu, rightMenu].map((menuGroup, index) => (
          <div key={index} className="flex flex-col space-y-4 w-full p-4 sm:p-5 bg-[#1c1208]/40 rounded-xl shadow-md backdrop-blur-md border border-[#dab67d]/30">
            {menuGroup.map((item, idx) => (
              <div key={idx} className="flex justify-between items-center w-full">
                <span className="font-semibold text-sm sm:text-base md:text-lg">{item.name}</span>
                <div className="flex-grow border-b mx-3 border-dashed border-[#dab67d]" />
                <span className="font-semibold text-sm sm:text-base md:text-lg">{item.price}</span>
              </div>
            ))}
          </div>
        ))}
